...
<?php
  ...
  if(isset($_POST['submit']))
  {
    ...
    $new_datedebut = date('d-m-Y', $date1);

    $date1 = strtotime($datefin); 
    $new_datefin = date('d-m-Y', $date1);

    $hotelid = $_SESSION['hotelid'];

    $sql = "SELECT * FROM Enregistrement WHERE Date >= '$datedebut' AND Date <= '$datefin' AND HotelID = '$hotelid'"; 
    
    $result = mysqli_query($conn, $sql);

    $sql1 = "SELECT * FROM Decaissement WHERE Date >= '$datedebut' AND Date <= '$datefin' AND HotelID = '$hotelid'"; 
    ...
    echo "<h1>Bilan du " . $new_datedebut . " au " . $new_datefin . "</h1>";
